...
def returned_inness(sampler, mask_idx, generator):
    ...
    noisy = inness.float() + 1e-3 * torch.rand(inness.shape, device=sampler.device, generator=generator)
...
def add_histograms(fig, inness, labels, class_names, extra, max_inness, row, col):
    ...
    for class_id in range(NUM_CLASSES):
        ...
        hover = f"{class_names[class_id]}<br>inness=%{{x:.1f}}<br>fraction=%{{y:.4f}}<extra></extra>"
        trace = go.Scattergl(x=bin_values, y=normalized_histogram(class_values, max_bin), mode="lines", name=class_names[class_id], legendgroup=f"class-{class_id}", showlegend=False, line={"color": "rgba(31, 119, 180, 0.22)", "width": 1}, hovertemplate=hover)
        fig.add_trace(trace, row=row, col=col)

    hover = "overall<br>inness=%{x:.1f}<br>fraction=%{y:.4f}<extra></extra>"
    trace = go.Scattergl(x=bin_values, y=normalized_histogram(inness, max_bin), mode="lines", name="overall", legendgroup="overall", showlegend=extra == EXTRAS[0], line={"color": "black", "width": 4}, hovertemplate=hover)
    fig.add_trace(trace, row=row, col=col)
...
def plot_symmetry(symmetry):
    ...
    subplot_titles = [f"extra={extra}, num_ret_tiles={NUM_TILES + extra}" for extra in EXTRAS]
    fig = make_subplots(rows=rows, cols=cols, shared_xaxes=True, subplot_titles=subplot_titles, vertical_spacing=0.035)
    sampler = SpurSampler(symmetry=symmetry, num_tiles=NUM_TILES, translation_canvas=TRANSLATION_CANVAS, num_ret_tiles=NUM_TILES, seed=SEED)
    if len(sampler) != NUM_MASKS or len(sampler.class_names) != NUM_CLASSES:
        raise RuntimeError(f"Expected {NUM_MASKS} masks in {NUM_CLASSES} classes, found {len(sampler)} masks in {len(sampler.class_names)} classes")
    ...
    for plot_index, extra in enumerate(EXTRAS):
        ...
        print(f"[symmetry {symmetry}] Sampling masks 0:{NUM_MASKS} with num_ret_tiles={num_ret_tiles} ...", flush=True)
        ...
        add_histograms(fig, inness.cpu().numpy(), labels, sampler.class_names, extra, 1.0, row + 1, col + 1)
        del inness

    fig.update_xaxes(title_text="inness", dtick=0.1)
    fig.update_yaxes(title_text="fraction of returned tiles")
    title = f"Symmetry {symmetry}: inness by class (num_tiles={NUM_TILES}, bin size={BIN_WIDTH})"
    fig.update_layout(title=title, height=350 * rows, width=1400, hovermode="closest", template="plotly_white")
